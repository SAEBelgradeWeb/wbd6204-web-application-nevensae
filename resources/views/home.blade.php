<x-app-layout>
    <x-slot name="header">
        <div class="h-16 p-4"
            ...
            <h2 class="text-3xl text-white">Home</h2>
        </div>
    </x-slot>
    ...
    <x-slot name="slot" >
        ...
        <div>
            ...
            @foreach($contests as $contest)
                <div class="m-8 w-1/3 flex" style="background-image: url({{$contest->image}}); background-position: center; background-repeat: no-repeat;">
                    <a href="/contests/{{$contest->id}}">Link to page</a>
                    <h4 class="align-middle mt-6 text-white inline-block">{{$contest->title}}</h4>
                    <button class="align-middle text-white">{{$contest->description}}</button>
                </div>
            @endforeach
                <div class="m-8">
